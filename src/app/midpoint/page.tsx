...
import { LatLngNullType } from '../types';
...
const MidpointPAge: FunctionComponent = () => {
  ...
  const [midpoint, setMidpoint] = useState<LatLngNullType>({ lat: null, lng: null });
    ...
    <PageContainer>
      <PageWrapper>
        ...
        <ControlWrapper>
          ...
          <Title>Середня точка:</Title>
          <TitleWrapper>
            {marker.lat1 && marker.lng1 && marker.lat2 && marker.lng2 ? (
              <div>
                <Paragraph>
                  Широта: <Underline>{midpoint.lat}</Underline>;
                </Paragraph>
                <br />
                <Paragraph>
                  Довгота: <Underline>{midpoint.lng}</Underline>;
                </Paragraph>
              </div>
            ) : (
              <Paragraph>Точки не вибрані</Paragraph>
            )}
            <IconButton
              onClick={() =>
                copyToClipboard(
